...
export async function refreshNews() {
  ...
  for (const source of newsSources) {
    ...
    try {
      ...
      const items = parseNewsFeed(html, source, at);
      if (items.length)
        await db.batch(
          items.map((item) => ({
            sql: 'INSERT INTO news (id,record,published_at) VALUES (?,?,?) ON CONFLICT(id) DO UPDATE SET record=excluded.record,published_at=excluded.published_at',
            params: [item.id, JSON.stringify(item), item.publishedAt || ''],
          })),
        );
      await recordSync(source.id, at, items.length, null);
