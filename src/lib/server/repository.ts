...
export async function database(){const db=await openStore();if(!ready)ready=(async()=>{await db.batch(schemaStatements.map(sql=>({sql})));await db.batch(verifiedCompanies.map(c=>({sql:'INSERT OR IGNORE INTO companies (slug,name,sector,area,kind,record,status,updated_at) VALUES (?,?,?,?,?,?,?,?)',params:[c.slug,c.name,c.sector,c.area,c.kind,JSON.stringify(c),'published',c.verifiedAt]})));})().catch(error=>{ready=undefined;throw error});await ready;return db;}
