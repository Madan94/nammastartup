    ...
    ready = (async () => {
      ...
      await db.batch(
        verifiedCompanies.map((c) => ({
          sql: "INSERT INTO companies (slug,name,sector,area,kind,record,status,updated_at) VALUES (?,?,?,?,?,?,?,?) ON CONFLICT(slug) DO UPDATE SET record=excluded.record WHERE companies.updated_at=excluded.updated_at AND companies.status='published'",
          params: [
            c.slug,
            c.name,
            c.sector,
...
export async function listCompanies(): Promise<Company[]> {
  ...
  return (
    await db.all<{ record: string }>(
      "SELECT record FROM companies WHERE status='published' ORDER BY name",
    )
  ).map((row) => JSON.parse(row.record));
}
export async function getCompany(slug: string): Promise<Company | null> {
  const db = await database();
  const [row] = await db.all<{ record: string }>(
    "SELECT record FROM companies WHERE slug=? AND status='published'",
    [slug],
  );
...
export async function listJobs(): Promise<JobListing[]> {
  ...
  return (
    await db.all<{ record: string }>(
      "SELECT jobs.record FROM jobs JOIN companies ON companies.slug=jobs.company_slug WHERE active=1 AND companies.status='published' ORDER BY observed_at DESC",
    )
  ).map((row) => JSON.parse(row.record));
}
export async function listNews(): Promise<NewsItem[]> {
  const db = await database();
  return (
    await db.all<{ record: string }>('SELECT record FROM news ORDER BY published_at DESC LIMIT 60')
  ).map((row) => JSON.parse(row.record));
}
