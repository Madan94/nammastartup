...
export async function POST(request: Request) {
  try {
    ...
    const company =
      body.action === 'approve'
        ? companySchema.parse({ ...(body.company as object), verifiedAt: new Date().toISOString() })
        : undefined;
    return Response.json(await reviewSubmission(body.id, body.action, body.note, company));
  } catch (error) {
