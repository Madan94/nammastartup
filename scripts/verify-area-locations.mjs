...
for (const area of [
  ...
  const url =
    ...
    encodeURIComponent(area + ', Chennai, Tamil Nadu');
  try {
    const response = await fetch(url, {
      headers: {
        'User-Agent': 'NammaStartupDirectory/1.0 (+https://github.com/Madan94/nammastartup)',
      },
    ...
    if (place) {
      ...
      if (latitude > 12.5 && latitude < 13.6 && longitude > 79.8 && longitude < 80.5)
        output[area] = {
          latitude,
          longitude,
    ...
    console.log(area, output[area] ?? 'No verified result');
  } catch (error) {
